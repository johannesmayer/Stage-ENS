...
def xy_magnetisation(spin_config):
    ...
    mag = numpy.sum(numpy.exp(1j*spin_config))
    return mag/float(N)
...
if len(sys.argv) != 5 :
    sys.exit("GIVE ME THE INPUT IN THE FORM: J : BETA : NUMBER OF CHAINS : CHAINLENGHT IN UNITS OF PI ")
...
for L in different_sizes:
    ...
    numpy.save(outdir + "/xy_grid_longruns_event_suscepts_beta_"+str(beta)+"_L_"+str(L)+".npy",all_suscepts)