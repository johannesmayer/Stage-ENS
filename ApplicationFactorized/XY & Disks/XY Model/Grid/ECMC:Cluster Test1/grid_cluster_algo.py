...
import math, numpy, random, time, sys, matplotlib.pylab as plt, os
...
for dummy_index in xrange(N_cluster_updates):
    ...
    if dummy_index % (float(N_cluster_updates)/100) == 0:
        print (str(dummy_index/float(N_cluster_updates)* 100)+"% in "+str(time.time()-starting_time)+" seconds")
    ...
    if numpy.dot(magn_pp,magn_pp) > 1:
        print 's',spins    
    	print 'mpp',magn_pp
...
print("Average cluster size: "+str(mean_cluster_size))
...
numpy.save(outdir+"/cluster_suscepts_L_"+str(L)+"_beta_"+str(beta)+"avg_c_"+str(mean_cluster_size)+".npy", all_suscepts)