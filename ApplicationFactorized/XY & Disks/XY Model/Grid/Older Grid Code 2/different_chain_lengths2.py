...
for setup in different_setups:
    ...
    config_filename = last_config_outdir+'/'+ID+'.npy'
    ...
    if read_from_file == True:
        logfile.write('INITIAL CONDITION FROM FILE \n')
    else:
        logfile.write('RANDOM INITIAL CONDITION \n')
    logfile.flush()
    ...
    for ith_chain in xrange(n_times):
        ...
        if (10 * ith_chain ) % n_times == 0:
            ...
            numpy.save(config_filename, spins)
            logfile.write('just saved the current configuration \n')
            logfile.write(80 * '*' + '\n')
