...
def calculate_angle(spin_config):
    my_angles = []
    for vector in spin_config:
        phi = numpy.arctan(vector[1]/vector[0])%(2*math.pi)
        my_angles.append(phi)
    return my_angles
...
snap_every_n_updates = 15
guessed_equil_time = 600

starting_time = time.clock()

outdir = "Grid_Data"
ID = 'cluster_configs_L_%i_beta_%f_snap_every_%i_updates' %(L,beta,snap_every_n_updates)
filename = ID + '.npy'
...
for dummy_index in xrange(N_cluster_updates):
    ...
    if dummy_index > guessed_equil_time and dummy_index % snap_every_n_updates== 0:
                all_snapshots.append(calculate_angle(spins))
