...
for my_file in file_list:
    if my_file.endswith('.data'):
        ...
        chain_length_label = 'L=' + my_file.split('_')[2] + ' l=' + my_file.split('_')[4] + ' ' + ' $\\beta$= ' + my_file.split('_')[3]
        filename = directory + '/' + my_file
        outfile = outdir + '/' + 'autocorr_' + my_file
        observable = numpy.loadtxt(filename, usecols = (0,))[thermal_cutoff:]
        ...
        neg_log_y = -1 * numpy.log(correlator[dt_min_fit:dt_max_fit])
        ...
        color = next(colormap)
        chain_length_label = chain_length_label + ' $\\tau= $%f'%tau_corr
