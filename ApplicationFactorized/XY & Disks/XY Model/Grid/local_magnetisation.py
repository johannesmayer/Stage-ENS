...
for i_sweep in range(n_times):
    ...
    while total_displacement < chain_length:
        ...
        lift = whos_next
        if all_moves % N == 0:
            current_local_magnet = numpy.zeros(N)
            for index in xrange(N):
                local_environment = []
                local_environment.append(spins[index])
                for neigh in nbr[index]:
                    local_environment.append(spins[neigh])
                current_local_magnet[index] = abs(xy_magnetisation(local_environment))
            all_local_magnets.append(current_local_magnet[:])
            
print("DURATION: "+str(time.time()-starting_time))
  
numpy.save("Grid Data/xy_grid_mean_local_magnet_"+str(beta)+"_L_"+str(L)+".npy",all_local_magnets)