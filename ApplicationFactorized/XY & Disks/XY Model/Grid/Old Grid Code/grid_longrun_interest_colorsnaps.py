...
list_conf = []
for index in xrange(len(my_data)-1):
    current_config = numpy.array(my_data[index][0])
    next_config = numpy.array(my_data[index+1][0])
    interesting_config = (next_config - current_config)%(2*math.pi) /math.pi
    list_conf.append(interesting_config)
...
for step in xrange(nsteps):
    ...
    var = pylab.imshow(conf,interpolation = 'nearest',vmin = 0, vmax = 2,cmap = pylab.get_cmap('bwr'))    
    fig.colorbar(var)    
    pylab.savefig('Interesting_snapshots/beta_'+sys.argv[2]+'_L_'+sys.argv[3]+'_snapshot_%06i.png' % step, bbox_inches='tight')
