...
spins = [random.uniform(0,0.01*math.pi) for k in range(N)]

for i_sweep in range(n_times):
    if i_sweep % (n_times/100.0) == 0:
        print("PROGRESS: "+str(100*i_sweep/float(n_times))+"% in "+str(time.time()- starting_time)+" seconds")
...
print len(last_config)
        
        
print("DURATION: "+str(time.time()- starting_time))            
numpy.save("Grid Data/xy_grid_final_config_beta_"+str(beta)+"_L_"+str(L)+".npy",last_config)